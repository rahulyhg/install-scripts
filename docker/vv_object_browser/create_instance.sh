...
parse_opts() {
    instance_name="Vedavaapi Object Browser"
    client_type="public"
    forward_port="8002"
    docker_project_name="vedavaapiObjectBrowser"
    redirect_uris="";
...
check_opts() {
    ...
    if [ \( -z "${admin_email}" \) -o \( -z "${admin_password}" \) -o \( -z "${docker_project_name}" \) -o \( -z "${main_platform_url_root}" \)  ]; then
